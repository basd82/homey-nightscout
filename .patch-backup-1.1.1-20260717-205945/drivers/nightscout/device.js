...
class NightscoutDevice extends Homey.Device {
  ...
  isTrend(direction) {
    const wanted = String(direction || '');
    if (!wanted) return false;

    if (this.lastDirection) {
      return this.lastDirection === wanted;
    }

    const current = this.getCapabilityValue('glucose_trend');
    return current === this.formatTrend(wanted);
  }
  ...
  async refresh() {
    ...
    try {
      ...
      if (d.direction) {
        this.lastDirection = String(d.direction);
